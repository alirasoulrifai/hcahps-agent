...
from config import (
    ...
    FACTS_IDS_PATH, SUMM_IDS_PATH, OLLAMA_URL, DEFAULT_LLM,
...
def ask_ollama(prompt: str, model: str = DEFAULT_LLM, temperature: float = 0.1) -> Iterator[str]:
    try:
...
def ask_ollama_str(prompt: str, model: str = DEFAULT_LLM, temperature: float = 0.1) -> str:
    """Non-streaming version for internal logic (decomposition, etc.)"""
    # Consume the generator
    return "".join(list(ask_ollama(prompt, model=model, temperature=temperature)))
